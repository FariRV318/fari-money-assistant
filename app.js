...
function buildPlan(){
 ...
 for(const d of flex.filter(x=>x.pieces===false&&!x.targetMonth)){if(remaining>=+d.balance){allocations.push({type:'Whole debt payment',name:d.name,amount:+d.balance,priority:d.priority,id:d.id});remaining-=+d.balance}}
...
function buildRoadmap(maxMonths=18){
 ...
 for(let i=0;i<maxMonths;i++){
  ...
  for(const d of debts.filter(x=>!x.fixed&&x.pieces===false&&x.balance>0&&x.targetMonth).sort((a,b)=>(a.targetMonth||'9999-99').localeCompare(b.targetMonth||'9999-99')||debtPriorityScore(b)-debtPriorityScore(a))){
    const targetDate=new Date(d.targetMonth+'-01T12:00:00');
    if(key<d.targetMonth){
  ...
  for(const d of debts.filter(x=>!x.fixed&&x.pieces===false&&x.balance>0&&!x.targetMonth).sort((a,b)=>debtPriorityScore(b)-debtPriorityScore(a))){if(available>=d.balance){const pay=d.balance;d.balance=0;available-=pay;payments.push({name:d.name,amount:pay,kind:'whole',note:'paid in full'})}}
...
function renderDebts(){const p=buildPlan();const road=buildRoadmap();$('#debtCards').innerHTML=state.debts.length?state.debts.map(d=>{
 ...
 const targetDate=d.targetMonth?new Date(d.targetMonth+'-01T12:00:00'):null;
 const targetRow=d.targetMonth?road.rows.find(m=>m.key===d.targetMonth):null;
 const reserveNow=d.targetMonth?road.rows[0]?.payments.find(x=>x.kind==='whole-reserve'&&x.name===d.name):null;
 ...
 let assistant='';
 if(d.pieces===false&&d.targetMonth){assistant=`Target: <strong>${monthLabel(targetDate)}</strong>. `;if(wholeMonth&&wholeMonth.key===d.targetMonth)assistant+=`Plan: pay ${esc(d.name)} in full in your chosen month.`;else if(targetShort)assistant+=`Your chosen month stays protected, but the current plan is ${esc(targetShort.note.replace(/^.*short /,''))} short. Reduce flexible spending or add income before then.`;else if(reserveNow)assistant+=`Reserve <strong>${money(reserveNow.amount)}</strong> now toward the full payment.`;else assistant+=`The assistant will keep this as the preferred full-payment deadline.`}
...
window.editDebt=id=>{const d=state.debts.find(x=>x.id===id);if(!d)return;editingDebtId=id;$('#debtName').value=d.name;$('#debtType').value=d.type;$('#debtBalance').value=d.balance;$('#debtFixed').value=d.fixed?'yes':'no';$('#debtMonthly').value=d.monthly;$('#debtDueDay').value=d.dueDay||1;$('#debtInterest').value=d.interest?'yes':'no';$('#debtRate').value=d.rate||0;$('#debtPriority').value=d.priority||'normal';$('#debtMinimum').value=d.minimum||0;$('#debtPieces').value=d.pieces===false?'no':'yes';$('#debtTargetMonth').value=d.targetMonth||'';$('#debtSubmitBtn').textContent='Update Debt';openModal('debt')};
